...
- **נוסף:** תמונת השראה לכל סגנון (נשלחת למודל כתמונה שנייה), בחירת סוג מאפה (כיכר גדולה, מאפה קטן, מאפה עם פירות או קרם), 1–4 גרסאות לכל תמונה, מונה התקדמות והורדת כל התוצאות כ־ZIP.
- **לא נבדק:** המנועים עם AI (Gemini ו־OpenAI) לא הורצו מול API אמיתי, כי לא היה מפתח. נבדק מצב השיפור המקומי, ובנוסף נבדק שהבקשות ל־Gemini ול־OpenAI נבנות נכון (סדר התמונות וההנחיה) מול שרת מדומה.
